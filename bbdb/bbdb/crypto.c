...
static X509 * load_root_certificate(void);
static crypto_page_t * crypto_get_page(crypto_t *crypto);
static crypto_page_t * crypto_page_init(crypto_t *crypto);
static int crypto_page_update_certificate(crypto_t *crypto, X509 *certificate);
static int crypto_signiture_get(crypto_t *crypto, uint8_t *buffer, int *sz);;
static int crypto_signiture_set(crypto_t *crypto, uint8_t *buffer, int sz);
static int fetch_certificate_from_page(crypto_t *crypto, crypto_page_t *cpage);
static int read_crypto_header(crypto_t *crypto);
static int validate_certificate(X509 *certificate, X509 *root_certificate);
static int write_crypto_header(crypto_t *crypto);
    ...
    crypto_t *crypto)
{
  ...
  records = crypto->index->record_num;
