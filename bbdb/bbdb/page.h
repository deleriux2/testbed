...
#define INDEX_HDR_OFFSET       32
#define CRYPTO_HDR_OFFSET      56
#define LICENSE_HDR_OFFSET     64
#define DENY_HDR_OFFSET        88
#define ALLOW_HDR_OFFSET      102
#define BBDB_HDR_OFFSET       110
...
typedef struct pagemap {
  int fd;
  int64_t size;
  /* This value only ever increments, its the last pageno +1 */
  int64_t pagecount;
  int64_t capacity;
  size_t filesize;
  bool ro;
...
static inline int64_t page_set_index_of(
    void *data,
    int64_t index)
{
  assert(data);
...
int page_move(pagemap_t *map, int64_t dst, int64_t src);
#undef CHECK_PAGE
#endif
