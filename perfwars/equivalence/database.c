    ...
    char type)
{
  ...
  rec->type = type;
  rec->datalen = len;
  memset(rec->data, 0, DB_MAX_DATA_SIZE);
  memcpy(rec->data, data, rec->datalen);
  if (rc == 1) 
    db->hdr->next_record_no = (recno+1);
    ...
    char *filename)
{
  ...
  if (rc) {
    rec = &db->records[recno];
    new = calloc(1, sizeof(db_record_t));
