		...
		ndmp_session *sess)
{
	...
	if (buinfo->error != NDMP_NO_ERR) {
      fprintf(stderr, "The server attribute packet returned an error: %s\n", ndmp_print_error(buinfo->error));
      goto fail;
	}

	sess->bklist_len = buinfo->butype_info.butype_info_len;
   sess->bklist = calloc(sess->bklist_len, sizeof(*sess->bklist));
	for (i=0; i < sess->bklist_len; i++) {
		strncpy(sess->bklist[i].backup_type, buinfo->butype_info.butype_info_val[i].butype_name, 256);
		sess->bklist[i].metadata_len = buinfo->butype_info.butype_info_val[i].default_env.default_env_len;
		sess->bklist[i].metadata = calloc(sizeof(*sess->bklist[i].metadata), sess->bklist[i].metadata_len);
		sess->bklist[i].attrs = buinfo->butype_info.butype_info_val[i].attrs;
		for (j=0; j < sess->bklist[i].metadata_len; j++) {
			strncpy(sess->bklist[i].metadata[j].name, 
				buinfo->butype_info.butype_info_val[i].default_env.default_env_val[j].name, 256);
         strncpy(sess->bklist[i].metadata[j].value, 
            buinfo->butype_info.butype_info_val[i].default_env.default_env_val[j].value, 256);
		}
	}
		...
		ndmp_session *sess)
{
	...
	for (i=0; i < fsinfo->fs_info.fs_info_len; i++) {
		...
		QUAD_TO_U64(sess->fslist[i].avail_size, t->avail_size);
