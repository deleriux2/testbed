    ...
    int32_t upidx)
{
  ...
  bbdb_ex_node_t *ex = (void *)bbdb->map + (BBDB_PAGESIZE * (bbdb->size));
    ...
    X509_STORE *trustdb)
{
  ...
  *bbdb->cryptindex = bbdb->size;
  ...
  sz = ((((sizeof(bbdb_crypto_entry_t) * bbdb->size) + sizeof(bbdb_crypto_table_t)) + bbdb->crypto->certsize + 1024) / BBDB_PAGESIZE) + 1;
  bbdb->crypto->size = sz;
    ...
    X509 *cert)
{
  ...
  if (!bbdb->license)
    bbdb_err(EXIT_FAILURE, "License table does not exist");
  EVP_VerifyUpdate(evp, bbdb->license, (BBDB_PAGESIZE * bbdb->license->size));
    ...
    X509_STORE *trustdb)
{
  ...
  *bbdb->pages = bbdb->size;
  ...
  map = mremap(bbdb->map, BBDB_CHUNKSIZE, BBDB_PAGESIZE * bbdb->size, 0);
  if (map == MAP_FAILED)
    err(EXIT_FAILURE, "Could not shrink mapping, commit failed");
  bbdb->map = map;
  if (ftruncate(bbdb->fd,  BBDB_PAGESIZE * bbdb->size) < 0)
