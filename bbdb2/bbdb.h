...
#define BBDB_CHUNKSIZE 0x140000000
...
typedef struct bbdb_sparse_entry_t {
  int32_t key;
  uint64_t index;
} bbdb_sparse_entry_t;

/* Sparse table representation */
typedef struct bbdb_sparse_table_t {
  uint64_t magic;
  int64_t size;
  /* The total remaining space in this block */
  int64_t capacity;
} bbdb_sparse_table_t;
...
typedef struct bbdb_t {
  ...
  uint64_t *rootindex;
  uint64_t *sparseindex;
  uint64_t *cryptindex;
  uint64_t *licenseindex;
  int64_t *numrecs;
  /* The top of the btree */
  bbdb_ex_node_t *root;
  bbdb_sparse_table_t *sparse;
...
void bbdb_build_sparse_table(bbdb_t *bbdb);
